...
const FILTERS = ["All", "Pure Veg", "Top Rated", "Under ₹100", "Fast Delivery", "New"];

function FoodDetail({ name }) {
  ...
  const displayName = name ? name.charAt(0).toUpperCase() + name.slice(1) : "Category";

  return (
    <div className="food-detail-page">
      {/* Filter bar */}
      <div className="fd-filter-bar">
        <div className="fd-filter-inner">
          {FILTERS.map(f => (
            <button key={f} className={`fd-filter-chip ${activeFilter === f ? "active" : ""}`} onClick={() => setActiveFilter(f)}>{f}</button>
          ))}
        </div>
      </div>
      ...
      <div className="fd-main">
        <div className="fd-top">
          <button className="fd-back-btn" onClick={() => router.back()}>← Back</button>
          <p className="fd-breadcrumb">Home / Food / <span>{displayName}</span></p>
        </div>
        ...
        {items.length === 0 ? (
          ...
          <div className="fd-grid">
            {items.map((item) => (
              <div key={item.id} className="fd-card" onClick={() => router.push(`/food/${name}/${item.id}`)}>
                <div className="fd-card-img">
                  <img src={item.img} alt={item.name} />
                  <span className="fd-card-badge">⭐ 4.{(item.id % 5) + 1}</span>
                  <div className="fd-card-offer">🏷️ 10% OFF</div>
                </div>
                <div className="fd-card-body">
                  <div className="fd-card-name">{item.category || item.name}</div>
                  <div className="fd-card-meta">
                    <span className="fd-card-price">{item.rating}</span>
                    <span className="fd-card-time">⏱ {item.time}</span>
                  </div>
                  <div className="fd-card-info">
                    <span className="fd-card-cat">{item.category.split(" ")[0]}</span>
                    <span className="fd-card-loc">📍 {item.location}</span>
                  </div>
                  <button className="fd-card-order" onClick={(e) => {
                    e.stopPropagation();
